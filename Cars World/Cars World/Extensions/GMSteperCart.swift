...
@IBDesignable public class GMStepperCart: UIControl {
    ...
    @objc public var labelFont = UIFont(name: "Raleway-Regular", size: 20.0)! {
        didSet {
    ...
    lazy var leftButton: UIButton = {
        ...
        button.addTarget(self, action: #selector(GMStepperCart.leftButtonTouchDown), for: .touchDown)
        button.addTarget(self, action: #selector(GMStepperCart.buttonTouchUp), for: .touchUpInside)
        button.addTarget(self, action: #selector(GMStepperCart.buttonTouchUp), for: .touchUpOutside)
        button.addTarget(self, action: #selector(GMStepperCart.buttonTouchUp), for: .touchCancel)
        return button
    }()
    ...
    lazy var rightButton: UIButton = {
        ...
        button.backgroundColor = self.buttonsBackgroundColor
        button.titleLabel?.font = self.buttonsFont
        button.addTarget(self, action: #selector(GMStepperCart.rightButtonTouchDown), for: .touchDown)
        button.addTarget(self, action: #selector(GMStepperCart.buttonTouchUp), for: .touchUpInside)
        button.addTarget(self, action: #selector(GMStepperCart.buttonTouchUp), for: .touchUpOutside)
        button.addTarget(self, action: #selector(GMStepperCart.buttonTouchUp), for: .touchCancel)
        return button
    ...
    lazy var label: UILabel = {
        ...
        let panRecognizer = UIPanGestureRecognizer(target: self, action: #selector(GMStepperCart.handlePan))
    ...
    fileprivate func setup() {
        ...
        //NotificationCenter.default.addObserver(self, selector: #selector(GMStepperCart.reset), name: UIApplication.willResignActiveNotification, object: nil)
    }
...
extension GMStepperCart {
    ...
    func scheduleTimer() {
        timer = Timer.scheduledTimer(timeInterval: timerInterval, target: self, selector: #selector(GMStepperCart.handleTimerFire), userInfo: nil, repeats: true)
    }
